...
<head>
    ...
    <style>
        :root {{
            --bg-primary: #f1f5f9; /* Más contraste para el proyector */
            --bg-sidebar: #ffffff;
            --bg-card: #ffffff;
            --border-color: #94a3b8; /* Borde más oscuro y visible */
            --text-main: #0f172a;
            --text-muted: #334155; /* Texto secundario más oscuro para legibilidad en proyector */
            
            --accent: #1d4ed8; /* Azul más oscuro de alto contraste */
            --accent-light: #dbeafe; /* Fondo claro de azul más pronunciado */
            --accent-hover: #1e40af;
...
<body>
    ...
    <div class="main-content">
        ...
        <div id="panel-overview">
            ...
            <div class="overview-map-grid">
                ...
                <div class="card" style="display:flex; flex-direction:column;">
                    ...
                    <div id="top-stations-list" style="overflow-y:auto; flex-grow:1; display:flex; flex-direction:column; gap:12px; max-height:400px; padding-right:4px;">
                        <!-- Populated by JS -->
                    </div>
            ...
            <div class="card" style="margin-top: 24px; padding: 20px;">
                <h3 style="font-size: 1rem; font-weight: 600; margin-bottom: 12px; display: flex; align-items: center; gap: 8px;">
                    🌍 Diagnóstico del Rendimiento Espacial y Geográfico
                </h3>
                <p style="font-size: 0.85rem; color: var(--text-muted); line-height: 1.6; margin-bottom: 12px;">
                    Al analizar la distribución del error absoluto medio (MAE) en el territorio, observamos una variación geográfica marcada en la efectividad de la corrección por Machine Learning. Los factores determinantes de este comportamiento incluyen:
                </p>
                <ul style="font-size: 0.85rem; color: var(--text-muted); line-height: 1.6; padding-left: 20px; display: flex; flex-direction: column; gap: 8px;">
                    <li>
                        <strong>Inercia y Relieve Topográfico:</strong> Las estaciones ubicadas en zonas con topografía irregular (valles del centro-oeste y áreas colindantes a las Grandes Llanuras) presentan inicialmente mayores errores sistemáticos en el modelo físico de la NWS debido a los efectos del relieve local y brisas de ladera. El modelo ML logra capturar estas pautas y reduce el error hasta en un <strong>50%</strong>.
                    </li>
                    <li>
                        <strong>Zonas de Transición Térmica:</strong> Los márgenes de mejora más altos coinciden con zonas propensas a fuertes inversiones térmicas diurnas (especialmente al amanecer y atardecer). El modelo físico tiende a subestimar la velocidad de calentamiento o enfriamiento del suelo en estas transiciones, mientras que XGBoost lo corrige integrando lags de temperatura e inercia histórica.
                    </li>
                    <li>
                        <strong>Llanuras Homogéneas:</strong> Por el contrario, en regiones con topografía plana y homogénea (donde la física simplificada del modelo NWS funciona de forma razonablemente correcta), la mejora relativa de ML es menor (20%-30%), confirmando que el valor agregado del modelo híbrido radica en mitigar la incapacidad de la física tradicional de modelar efectos microclimáticos locales.
                    </li>
                </ul>
            </div>
        </div>
        ...
        <div id="panel-monitoring" style="display:none; flex-direction:column; gap:24px;">
            ...
            <!-- Grid de Contenido Principal (Gráficos + SHAP Lateral) -->
            <div class="tab-panel-monitoring">
        ...
        <div id="panel-explainability" style="display:none;">
            ...
            <div class="explainability-grid" style="grid-template-columns: 1fr; margin-bottom:32px;">
                ...
                <div class="card" style="padding:24px;">
                    ...
                    <p style="font-size:0.85rem; color:var(--text-muted); margin-bottom:24px; line-height:1.5;">
                        Las gráficas inferiores representan la contribución global de cada variable predictora en los modelos entrenados. La <strong>Importancia Interna de XGBoost</strong> mide la ganancia promedio que cada variable aporta al construir las divisiones de los árboles de decisión. La <strong>Importancia por Permutación (PFI)</strong> evalúa de manera más directa el decremento medio en el error absoluto (MAE) del modelo cuando los valores de la característica se desordenan aleatoriamente.
                    </p>
                    
                    <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(350px, 1fr)); gap:24px;">
                        <div style="background:#f8fafc; border:1px solid var(--border-color); border-radius:8px; padding:16px;">
                            <h4 style="font-size:0.9rem; font-weight:600; margin-bottom:12px; text-align:center; color:#f43f5e;">
                                🌡️ Importancia de Variables - Temperatura
                            </h4>
                            <div id="feature-importance-temp-chart" style="height:350px; width:100%;"></div>
                        </div>
                        <div style="background:#f8fafc; border:1px solid var(--border-color); border-radius:8px; padding:16px;">
                            <h4 style="font-size:0.9rem; font-weight:600; margin-bottom:12px; text-align:center; color:#3b82f6;">
                                💧 Importancia de Variables - Humedad Relativa
                            </h4>
                            <div id="feature-importance-hum-chart" style="height:350px; width:100%;"></div>
                        </div>
                    </div>
                    
                    <p style="font-size:0.75rem; color:var(--text-muted); line-height:1.4; margin-top:16px; border-top:1px solid var(--border-color); padding-top:12px;">
    ...
    <script>
        ...
        function changeTab(tabName) {{
            ...
            document.getElementById('panel-monitoring').style.display = tabName === 'monitoring' ? 'flex' : 'none';
        ...
        function drawPlots() {{
            ...
            const layoutCommon = {{
                ...
                xaxis: {{ gridcolor: '#cbd5e1', linecolor: '#475569', tickfont: {{ color: '#0f172a', size: 10 }} }},
                yaxis: {{ gridcolor: '#cbd5e1', linecolor: '#475569', tickfont: {{ color: '#0f172a', size: 10 }} }}
            }};
        ...
        function drawGlobalImportance() {{
            ...
            // Separar y ordenar para temperatura (excluyendo el pronóstico base temp_fcst)
            const sortedTemp = DB.features.map((f, idx) => ({{
                id: f,
                label: FEATURE_LABELS[f] || f,
                val: tempValues[idx]
            }}))
            .filter(item => item.id !== 'temp_fcst')
            .sort((a, b) => b.val - a.val);
            
            // Separar y ordenar para humedad (excluyendo el pronóstico base hum_fcst)
            const sortedHum = DB.features.map((f, idx) => ({{
                id: f,
                label: FEATURE_LABELS[f] || f,
                val: humValues[idx]
            }}))
            .filter(item => item.id !== 'hum_fcst')
            .sort((a, b) => b.val - a.val);
            
            const traceTemp = {{
                y: sortedTemp.map(c => c.label),
                x: sortedTemp.map(c => c.val),
                name: 'Modelo Temp.',
                type: 'bar',
                orientation: 'h',
                marker: {{ color: '#f43f5e' }}
            ...
            const traceHum = {{
                y: sortedHum.map(c => c.label),
                x: sortedHum.map(c => c.val),
                name: 'Modelo Hum.',
                type: 'bar',
                orientation: 'h',
                marker: {{ color: '#3b82f6' }}
            ...
            const titleX = state.impType === 'xgb' ? 'Ganancia del Árbol (Gain)' : 'Decremento del Score (MAE)';
            ...
            const layoutTemp = {{
                ...
                xaxis: {{ gridcolor: '#f1f5f9', title: titleX + ' (°C)' }}
            }};
            ...
            const layoutHum = {{
                ...
                paper_bgcolor: 'rgba(0,0,0,0)',
                yaxis: {{ autorange: 'reverse', tickfont: {{ size: 9 }} }},
                xaxis: {{ gridcolor: '#f1f5f9', title: titleX + ' (%)' }}
            }};
            
            Plotly.newPlot('feature-importance-temp-chart', [traceTemp], layoutTemp, {{ responsive: true }});
            Plotly.newPlot('feature-importance-hum-chart', [traceHum], layoutHum, {{ responsive: true }});
        }}
